...
async function downloadPdf(order){
  ...
  try{
    ...
    if(Number(order.shippingWeightKg||0)>0) line(language()==='it'?'Peso spedizione stimato':(l==='es'?'Peso estimado del envío':l==='fr'?'Poids estimé de l’envoi':l==='de'?'Geschätztes Versandgewicht':'Estimated shipping weight'),new Intl.NumberFormat(LOCALE[l]||LOCALE.it,{minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(order.shippingWeightKg))+' kg');
