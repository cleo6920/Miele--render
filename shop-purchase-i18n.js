...
de:{
 ...
 missing:'Fülle alle Pflichtfelder aus, bevor du fortfährst.',invalidEmail:'Gib eine gültige E-Mail-Adresse ein.',invalidCap:'Gib eine gültige Postleitzahl für das gewählte Land ein.',invalidAddress:'Gib Straße und Hausnummer vollständig ein, zum Beispiel „Via Roma 12“.',addressVerify:'Die Adresse kann nicht geprüft werden. Kontrolliere Straße, Hausnummer, PLZ, Ort und Land.',emptyCart:'Dein Warenkorb ist leer.',sendError:'Die Bestellung konnte nicht gesendet werden. Bitte versuche es erneut.',timeout:'Das Senden dauert zu lange. Deine Bestellung wurde nicht gelöscht: Bitte versuche es gleich noch einmal.',network:'Der Bestellservice ist momentan nicht erreichbar. Bitte versuche es gleich noch einmal.',
...
fr:{
 country:'Pays de livraison *',phonePrefix:'Indicatif',phoneNumber:'Numéro',selectCountry:'Sélectionnez le pays',province:'Province / Région',provincePh:'ex. MN / Madrid',invalidProvince:"Pour l’Italie, saisissez le code de province à 2 lettres.",invalidPhone:'Saisissez un numéro de téléphone valide pour le pays sélectionné.',shippingPending:'À confirmer',internationalReason:'Livraison internationale : le coût sera confirmé avant traitement.',goodsTotalPending:'Total produits · livraison à confirmer',shippingCalcError:'Impossible de calculer la livraison pour le moment.',posteItalyReason:kg=>'Poste Delivery Web · poids estimé '+kg.toFixed(2)+' kg',posteIntlReason:(kg,zone)=>'Poste Delivery International Standard · poids estimé '+kg.toFixed(2)+' kg · Zone '+zone,postePendingReason:kg=>'Poste Italiane · poids estimé '+kg.toFixed(2)+' kg · tarif à confirmer pour cette destination',
 ...
 missing:'Complétez tous les champs obligatoires avant de continuer.',invalidEmail:'Saisissez une adresse e-mail valide.',invalidCap:'Saisissez un code postal valide pour le pays sélectionné.',invalidAddress:'Saisissez la rue et le numéro complet, par exemple « Via Roma 12 ». ',addressVerify:"Impossible de vérifier l’adresse. Contrôlez la rue, le numéro, le code postal, la ville et le pays.",emptyCart:'Votre panier est vide.',sendError:"La commande n’a pas pu être envoyée. Réessayez.",timeout:"L’envoi prend trop de temps. Votre commande n’a pas été supprimée : réessayez dans un instant.",network:"Impossible de joindre le service de commande pour le moment. Réessayez dans un instant.",
...
async function downloadPdf(order){
  ...
  try{
    ...
    function header(){
      ...
      doc.setTextColor(255,255,255);doc.setFont('helvetica','bold');doc.setFontSize(17);doc.text('LA FABBRICA DELLE API',M,14);
      doc.setFont('helvetica','normal');doc.setFontSize(9);doc.text(pdfSafe(x.pdfTitle),M,22);
    ...
    if(Number(order.shippingWeightKg||0)>0) line(language()==='it'?'Peso spedizione stimato':(l==='es'?'Peso estimado del envío':l==='fr'?'Poids estimé de l’envoi':l==='de'?'Geschätztes Versandgewicht':'Estimated shipping weight'),Number(order.shippingWeightKg).toFixed(2)+' kg');
    ...
    need(22);doc.setTextColor(95,105,99);doc.setFont('helvetica','normal');doc.setFontSize(8.5);
    const contact=doc.splitTextToSize(pdfSafe(x.pdfContact),W-2*M);doc.text(contact,M,y);y+=contact.length*4+3;
    doc.setFont('helvetica','bold');doc.text(pdfSafe(x.pdfNonFiscal),M,y);
    footer();
